...
export const addCategorySchema = yup.object().shape({
  name: yup
    .string()
    .matches(vietnameseNameRegex, 'Tên danh mục không được chứa ký tự đặc biệt')
    .min(2, 'Tên danh mục phải có ít nhất 2 ký tự')
    .max(50, 'Tên danh mục không dài quá 50 ký tự')
  ...
  type: yup.string().required('Loại danh mục không được để trống')
})
